...
def train_model(actor, critic, actor_memory, critic_memory, actor_optim, critic_optim, param_range):
    # step 1: train critic several steps with respect to returns
    states, actions, rewards, masks = split_memory(critic_memory)
    values = critic(torch.Tensor(states))
    returns, advants = get_gae(rewards, masks, values)

    states = states[::hp.drop_num]
    returns = returns[::hp.drop_num]
    advants = advants[::hp.drop_num]

    train_critic(critic, states, returns, advants, critic_optim)

    with torch.no_grad():
        for name, param in critic.named_parameters():
            if "bias" in name: # according to the paper
                continue
            param.data.clamp_(param_range[0][name], param_range[1][name]) # TODO: using other projections

    # step 2: train the actor
    states, actions, rewards, masks = split_memory(actor_memory)
    values = critic(torch.Tensor(states))
    returns, advants = get_gae(rewards, masks, values)

    states = states[::hp.drop_num]
    actions = actions[::hp.drop_num]
    returns = returns[::hp.drop_num]
    advants = advants[::hp.drop_num]
    ...
    # step 3: do backtracking line search for n times
