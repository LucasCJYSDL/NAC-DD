...
def get_param_range(q_network, max_param_szie):
    param_min, param_max = {}, {}
    with torch.no_grad():
        for name, param in q_network.named_parameters():
            param_min[name] = param.data.clone() - max_param_szie
            param_max[name] = param.data.clone() + max_param_szie
...
def split_memory(memory):
    ...
    return tuple([np.array(entry_list) for entry_list in entry_lists])
